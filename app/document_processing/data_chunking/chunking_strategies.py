...
try:
    ...
    # Tạo dummy numpy để tránh lỗi import khi không có sentence_transformers
    class DummyNumpyArray:
        pass
    
    class DummyNumpy:
        ndarray = DummyNumpyArray
        def mean(self, x):
...
class SemanticStrategy(ChunkingStrategy):
    ...
    def _find_semantic_breakpoints(self, embeddings, sentences: List[str]) -> List[int]:
        """Tìm điểm cắt dựa trên thay đổi ngữ nghĩa."""
        breakpoints = [0]  # Bắt đầu luôn là breakpoint
        
        # Kiểm tra nếu embeddings là numpy array thực sự
        if not SENTENCE_TRANSFORMERS_AVAILABLE:
            # Fallback: chia đều
            chunk_size_in_sentences = max(1, len(sentences) // 5)
            for i in range(chunk_size_in_sentences, len(sentences), chunk_size_in_sentences):
